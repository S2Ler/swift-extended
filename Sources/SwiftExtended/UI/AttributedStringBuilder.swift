...
public final class AttributedStringBuilder {
  ...
  /// Initialise builder with initial string.
  public init(_ initialString: NSMutableAttributedString) {
    self.storage = initialString
  }
  ...
  /// Applies `attributes` for chars in range which is obtained from `rangeBlock`.
  /// - Parameters:
  ///   - rangeBlock: A block which should return NSRange to apply `attributes` to.
  ///                 The current attributed string is passed to this block.
  @discardableResult
  public func addAttributes(_ attributes: [NSAttributedString.Key: Any],
                            rangeBlock: (NSAttributedString) -> NSRange) -> Self {
    let range = rangeBlock(makeCurrentStorageCopy())
    storage.addAttributes(attributes, range: range)
    return self
  }

  private func makeCurrentStorageCopy() -> NSAttributedString {
